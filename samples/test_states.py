...
from .models import LysateBatch, ExtractionBatch, LibraryBatch
...
class LibraryBatchStateTest(SimpleTestCase):
	def match_state_and_string(self, state_int, state_name):
		x = LibraryBatch()
		x.status = state_int
		self.assertEqual(x.get_status(), state_name)
	...
	def test_recall_state_open(self):
		self.match_state_and_string(LibraryBatch.OPEN, 'Open')
		
	def test_recall_state_inprogress(self):
		self.match_state_and_string(LibraryBatch.IN_PROGRESS, 'In progress')
	
	def test_recall_state_closed(self):
		self.match_state_and_string(LibraryBatch.CLOSED, 'Closed')
		
	def test_recall_state_stop(self):
		self.match_state_and_string(LibraryBatch.STOP, 'Stop')
