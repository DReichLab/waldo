...
class Sample(Timestamped):
	...
	def get_group_label(self):
		# manually set group label overrides automatic generation
		if len(self.group_label) > 0:
...
def create_lysate(lysate_layout_element, lysate_batch, user):
	...
	# only create the lysate from powder for this well once
		...
		powder_used_mg = get_value(lysate_layout_element, 'powder_used_mg', default=0)
	...
	def requires_p5_index_start(self):
		for layout_element in self.layout_elements():
			if layout_element.control_type == None and get_value(layout_element, 'library', 'library_batch', 'protocol', 'library_type') == 'ds' and (layout_element.p5_index is None or layout_element.p7_index is None):
				return True
		return False # single stranded or empty or double-stranded indices are assigned
...
class SequencingRun(Timestamped):
	...
	def i5_indices(self):
		...
		for sequenced_library in SequencedLibrary.objects.filter(sequencing_run=self):
			sequence = get_value(sequenced_library.indexed_library, 'get_i5', 'sequence', default=None)
	...
	def i7_indices(self):
		...
		# indices for this sequencing run
		for sequenced_library in SequencedLibrary.objects.filter(sequencing_run=self):
			sequence = get_value(sequenced_library.indexed_library, 'get_i7', 'sequence', default=None)
			if sequence is not None and sequence not in indices:
				indices.append(sequence)
		return indices
