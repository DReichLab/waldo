...
class Timestamped(models.Model):
	creation_timestamp = DateTimeField(default=timezone.now)
	created_by = models.CharField(max_length=50)
	modification_timestamp = DateTimeField(default=timezone.now)
...
class Return(Timestamped):
	...
	return_date = DateField(default=date.today)
	return_method = models.CharField(max_length=50)
	tracking_number = models.CharField(max_length=100)
	courier_delivery_date = DateField(default=date.today)
	recipient_delivery_confirmation = models.CharField(max_length=100)
...
class Sample(Timestamped):
	...
	return = models.ForeignKey(Return, on_delete=models.PROTECT)

	individual_id = CharField(max_length=15)
	
	skeletal_element = models.CharField(max_length=150, help_text='Type of bone sample submitted for aDNA analysis')
	skeletal_code = models.CharField(max_length=30, help_text='Sample identification code assigned by the collaborator')
...
class Collaborator(Timestamped):
	...
	title = models.CharField(max_length=30, help_text="Collaborator's professional title", blank=True)
	institution = models.CharField(max_length=100, db_index=True, help_text="Collaborator's associated institution or company")
	department = models.CharField(max_length=100, help_text="Collaborator's department or division", blank=True)
	...
	phone_number_office = models.CharField(max_length=25, blank=True)
	phone_number_mobile = models.CharField(max_length=25, blank=True)
	email_1 = models.CharField(max_length=50)
	email_2 = models.CharField(max_length=50, blank=True)
	skype_user_name = models.CharField(max_length=50, blank=True)
	facetime_user_name = models.CharField(max_length=50, blank=True)
	whatsapp_user_name = models.CharField(max_length=50, blank=True)
...
class PowderSample(Timestamped):
	...
	total_mg_powder_produced = models.FloatField(help_text='Total miligrams of bone powder produced from the sample')
	storage_location = models.CharField(max_length=50)
	
class Lysate(Timestamped):
	powder_sample = models.ForeignKey(PowderSample)
	mg_powder_used = models.FloatField(help_text='milligrams of bone powder used in lysis')
...
class ExtractBatch(Timestamped):
	...
	protocol = models.ForeignKey, on_delete=models.PROTECT)
	technician = models.CharField(max_length=50)
	date = models.DateField()
...
class ExtractionProtocol(Timestamped):
	...
	manual_robotic = models.CharField() # TODO
	...
	binding_buffer = models.CharField(max_length=50)
...
class LibraryBatch(Timestamped):
	...
	prep_robot = models.CharField(max_length=50)
...
class MTCapturePlate(Timestamped):
	name = models.CharField(max_length=50)
	protocol = models.ForeignKey(MTCaptureProtocol, on_delete=models.PROTECT)
	technician = models.CharField(max_length=50)
	date = models.DateField()
	robot = models.CharField(max_length=50)
...
class NuclearCapturePlate(Timestamped):
	...
	protocol = models.ForeignKey(NuclearCaptureProtocol, on_delete=models.PROTECT)
	technician = models.CharField(max_length=50)
	date = models.DateField()
	robot = models.CharField(max_length=50)
...
class RadiocarbonDatedSample(Timestamped):
	...
	radiocarbon_shipment = ForeignKey(RadiocarbonShipment, on_delete=models.PROTECT)
