...
class SampleSummaryLookupForm(forms.Form):
	...
	external_id = ModelChoiceField(
		queryset=Sample.objects.all(),
		widget=TextInput,
		help_text='External ID for sample',
		required=False,
	...
	def __init__(self, *args, **kwargs):
		...
		if self.instance.pk:
			...
			if sample:
				...
				self.fields['location'].initial = get_value(sample, 'location_str')
...
class LibraryForm(UserModelForm):
	...
	class Meta:
		model = Library
		fields = ['well_position', 'reich_lab_library_id', 'p5_index', 'p7_index', 'p5_barcode', 'p7_barcode', 'nanodrop', 'qpcr_ds', 'qpcr_assay_a_ss', 'qpcr_assay_b_ss', 'assay_a_percent_inhibition', 'assay_b_total_molecules', 'plate_id', 'fluidx_barcode', 'notes']
		widgets = {
			'notes': Textarea(attrs={'cols': 60, 'rows': 2}),
		}
	...
	def __init__(self, *args, **kwargs):
		...
		for option in ['p5_index', 'p7_index', 'p5_barcode', 'p7_barcode', 'nanodrop', 'qpcr_ds', 'qpcr_assay_a_ss', 'qpcr_assay_b_ss', 'assay_a_percent_inhibition', 'assay_b_total_molecules']:
			self.fields[option].required = False
		if self.instance:
...
class MobLayoutUploadForm(forms.Form):
	...
	external_extraction_lab = forms.CharField(required=False, initial='', help_text='For samples arriving as extracts, the originating lab. These have the sample primary key without an S (not the Reich lab ID) in the Extract column')
...
class SampleForm(UserModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		for option in ['reich_lab_id', 'special_restrictions', 'collaborator', 'collection_keeper', 'excavator', 'loan_expiration_date']:
			self.fields[option].disabled = True
			self.fields[option].required = False
		for option in ['special_restriction', 'date_start', 'date_end', 'average_bp_date', 'date_stdev', 'excavation_year']:
			self.fields[option].required = False
...
class PublicationForm(UserModelForm):
	...
	class Meta:
		model = Publication
		fields = ['abbreviation', 'title', 'first_author', 'year', 'journal', 'pages', 'author_list', 'url', 'publication_type', 'notes', 'is_draft']
